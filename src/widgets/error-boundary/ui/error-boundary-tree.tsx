...
import { ERROR_BOUNDARY_TREE_ITEMS } from "../model";

function ErrorBoundaryTreeComponent() {
	const t = useTranslations("error_boundary");

	const tree = useTree<ITreeItemData>({
		dataLoader: {
			getChildren: (itemId) =>
				ERROR_BOUNDARY_TREE_ITEMS[itemId]?.children ?? [],
			getItem: (itemId) => ERROR_BOUNDARY_TREE_ITEMS[itemId]
		},
		features: [syncDataLoaderFeature, hotkeysCoreFeature],
		getItemName: (item) => item.getItemData().name,
		indent: 20,
		initialState: {
			expandedItems: ["root", "shared", "ui", "eb"]
		},
...
export const ErrorBoundaryTree = withErrorBoundary(ErrorBoundaryTreeComponent);
