...
export class FsdAgent {
	public async analyze(
		files: Array<string | { path: string; imports?: string[] }>
	): Promise<IViolation[]> {
		...
		for (const fileItem of files) {
			...
			const imports: string[] =
				typeof fileItem === "string" ? [] : fileItem.imports || [];

			const fromLayer = layers.find((l: string) =>
				path.includes(`/${l}/`)
			);
			...
			for (const imp of imports) {
				const toLayer = layers.find(
					(l: string) =>
						imp.includes(`/${l}/`) || imp.startsWith(l + "/")
				);
				if (!toLayer) continue;
				...
				if (!allowed.includes(toLayer) && toLayer !== fromLayer) {
					violations.push({
						...
						suggestion:
							"Move the shared logic to Shared layer or refactor the dependency."
					});
				}
				...
				if (
					!isShared &&
					defaultRules?.fsd?.rules?.["enforce-barrel-exports"]
				) {
