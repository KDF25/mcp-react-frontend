...
export class StructureAgent {
	public async checkModuleStructure(
		modulePath: string,
		files: string[]
	): Promise<IViolation[]> {
		...
		if (modelFiles.length > 0) {
			const rootModelFiles = modelFiles.filter(
				(f) => !f.split("/model/")[1].includes("/")
			);
		...
		if (hasApiSegment && structure.converters.mandatory) {
			const hasConverter = files.some((f) =>
				f.endsWith(structure.converters.suffix)
			);
			if (!hasConverter) {
				violations.push({
					agent: ENUM_AGENT_TYPE.STYLE_ENFORCER,
