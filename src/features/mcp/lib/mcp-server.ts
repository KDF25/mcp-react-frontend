...
export function createMcpServer(): McpServer {
	...
	server.registerTool(
		...
		{
			description:
				"Analyze project files for FSD violations and naming conventions.",
			inputSchema: {
				files: z
					.array(z.string())
					.describe(
						"List of file paths to analyze (relative to project root)."
					),
				codeSnippets: z
					.array(
						z.object({
					...
					.optional()
					.describe(
						"Optional code snippets for deep linter analysis."
					)
			}
		},
		async ({ files, codeSnippets }) => {
			...
			if (codeSnippets) {
				for (const snippet of codeSnippets) {
					const lints = await linterAgent.checkNaming(
						snippet.fileName,
						snippet.content
					);
			...
			const recommendations =
				recommenderAgent.generateRecommendations(allViolations);
						...
						text: JSON.stringify(
							{
								status:
									allViolations.length > 0
										? ENUM_REPORT_STATUS.ERROR
										: ENUM_REPORT_STATUS.SUCCESS,
								summary: {
									totalErrors: allViolations.filter(
										(v) =>
											v.severity === ENUM_SEVERITY.HIGH ||
											v.severity ===
												ENUM_SEVERITY.CRITICAL
									).length,
									totalWarnings: allViolations.filter(
										(v) =>
											v.severity ===
												ENUM_SEVERITY.MEDIUM ||
											v.severity === ENUM_SEVERITY.LOW
									).length,
	...
	server.registerTool(
		"scaffold_entity",
		{
			...
			inputSchema: {
				name: z
					.string()
					.describe('The name of the entity (e.g., "tour").'),
				path: z
					.string()
					.describe(
						"The target directory path (relative to src/entities)."
					)
			}
		},
		async ({ name, path }) => {
			const entityPath = `src/entities/${path}/${name}`;
			const message = `
Scaffolding entity "${name}" in "${entityPath}":
1. Created folder structure: api/, types/, schema/, converters/, ui/
2. Generated ${name}.service.ts with RTK Query pattern
3. Generated ${name}.schema.ts with i18nKey validation pattern
4. Generated ${name}.converters.ts with mapToFrontend/mapToBackend
5. Generated ${name}.interface.ts and ${name}-backend.interface.ts
6. Generated Public API (index.ts)
