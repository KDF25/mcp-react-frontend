...
export class ProjectValidator {
	...
	public async validateProject(rootPath: string): Promise<IViolation[]> {
		...
		for (const filePath of tsFiles) {
			...
			const namingViolations = await this.linterAgent.checkNaming(
				filePath,
				content
			);
			...
			const imports = sourceFile
				...
				.map((id) => id.getModuleSpecifierValue());
			const fsdViolations = await this.fsdAgent.analyze([
		...
		for (const mod of modules) {
			...
			const structViolations =
				await this.structureAgent.checkModuleStructure(
					mod,
					moduleFiles
				);
