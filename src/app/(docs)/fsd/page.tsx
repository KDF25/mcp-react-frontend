...
export default function FsdPage() {
	...
	const entityTree = useTree<Item>({
		dataLoader: {
			getChildren: (itemId) =>
				entityStructureItems[itemId].children ?? [],
	...
	const widgetTree = useTree<Item>({
		dataLoader: {
			getChildren: (itemId) =>
				widgetStructureItems[itemId].children ?? [],
		...
		<>
			...
			<div className="space-y-12">
				...
				<section id="entities-structure">
					...
					<div className="space-y-4">
						<p className="text-muted-foreground text-sm">
							Каждая сущность (entity) — это самодостаточный
							модуль. Мы используем строгое разделение по
							подпапкам для предсказуемости и простоты поддержки.
						</p>
						...
						<div className="grid gap-4 md:grid-cols-2 mt-4">
							<ul className="space-y-2 text-sm text-muted-foreground">
								...
								<li className="flex gap-2">
									...
									Разделение на интерфейсы и типы для чистоты
									кода.
								</li>
								<li className="flex gap-2">
									<strong className="text-foreground">
				...
				<section id="widgets-structure">
					...
					<div className="space-y-4">
						<p className="text-muted-foreground text-sm">
							Виджеты — это "умные" блоки. Мы отделяем логику
							(model) от визуальной части (ui), чтобы упростить
						...
						<Card className="border-primary/10 bg-primary/5">
							<CardContent className="p-4 text-sm">
								<p>
									...
									<code>model/</code> содержит всё то, что
									делает виджет функциональным (схемы Zod,
									конфиги форм, локальные типы). Папка{" "}
									<code>ui/</code> содержит только компоненты
									отображения.
								</p>
							</CardContent>
						</Card>
